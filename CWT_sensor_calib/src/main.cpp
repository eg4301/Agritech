...
void receive_reading(int num_bytes, byte reading[]);
...
void request_reading(byte address) {
  ...
  command[5] = 0x04;     // Number of registers, low byte
...
void reading_rika() {
  ...
  command[0] = 0x06;  // Set the address of the water meter based on last 2 digits
  command[1] = 0x03;     // Set the function code for reading data register, given in datasheet
  command[2] = 0x00;     // Starting address, high byte
  command[3] = 0x00;     // Starting address, low byte
  command[4] = 0x00;     // Number of registers, high byte
  ...
  receive_reading(13, reading);
    
   //return the bytes with values for comparison
  for (int i = 0; i < 4; i++ ){
    values[i] = reading[i];
  }
